<!DOCTYPE html>
<html>
<head>
    <title></title>
    <!-- Metadata -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <!-- Favicon -->
    <link rel="icon" type="image/png" href="assets/images/ico/favicon.png" />
    <!-- CSS Plugins -->
    <link rel="stylesheet" href="assets/css/plugins.min.css">
    <link rel="stylesheet" href="assets/css/fontawesome.css">
    <link rel="stylesheet" href="assets/css/brands.css">
    <link rel="stylesheet" href="assets/css/solid.css">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.11.2/css/all.css" integrity="sha384-KA6wR/X5RY4zFAHpv/CnoG2UW1uogYfdnP67Uv7eULvTveboZJg0qUpmJZb5VqzN" crossorigin="anonymous">

    <link rel="stylesheet" href="assets/css/sweetalert.css">
    <!-- Custom CSS File -->

    <link rel="stylesheet" href="assets/css/style.css">
    <link rel="stylesheet" type="text/css" href="assets/css/share.css" />
    <!--for custom and Imgur-->
    <link href="./css/style.css" rel="stylesheet" media="screen">
    <link href="./css/mobile-style.css" rel="stylesheet" media="screen">
<!--    <link rel="shortcut icon" href="./favicon.png">-->
</head>
<body>
<div class="ms-main-container">
    <!-- Preloader -->
<!--    <div class="ms-preloader"></div>-->
    <!-- Header -->
    <header class="ms-header navbar-white">
        <nav class="ms-nav">
            <div class="ms-logo">
                <a href="index" data-type="page-transition">
                    <div class="logo-dark"><img src="assets/images/logo.png" alt="logo image"></div>
                    <div class="logo-light current"><img src="assets/images/logo.png" alt="logo image"></div>
                </a>
            </div>
            <button class="hamburger" type="button" data-toggle="navigation">
          <span class="hamburger-box">
            <span class="hamburger-label">menu</span>
            <span class="hamburger-inner"></span>
          </span>
            </button>
            <div class="height-full-viewport">
                <ul class="ms-navbar">
                    <!-- Nav Link -->
                    <li class="nav-item">
                        <a href="index" data-type="page-transition">
                            <span class="ms-btn">home</span>
                            <span class="nav-item__label">Back to home page</span>
                        </a>
                    </li>
                    <!-- Nav Link -->
                    <li class="nav-item">
                        <a href="paintings" data-type="page-transition">
                            <span class="ms-btn">paintings</span>
                            <span class="nav-item__label">View my works</span>
                        </a>
                    </li>
                    <!-- Nav Link -->
<!--                    <li class="nav-item">-->
<!--                        <a href="gallery" data-type="page-transition">-->
<!--                            <span class="ms-btn">gallery</span>-->
<!--                            <span class="nav-item__label">Interactively disintermediate</span>-->
<!--                        </a>-->
<!--                    </li>-->
                    <!-- Nav Link -->
                    <li class="nav-item">
                        <a href="shows" data-type="page-transition">
                            <span class="ms-btn">Exhibitions</span>
                            <span class="nav-item__label">Discover fascinating stories</span>
                        </a>
                    </li>
                    <!-- Nav Link -->
                    <li class="nav-item">
                        <a href="about" data-type="page-transition">
                            <span class="ms-btn">about</span>
                            <span class="nav-item__label">Learn more about the me</span>
                        </a>
                    </li>
                    <!-- Nav Link -->
                    <li class="nav-item">
                        <a href="contact" data-type="page-transition">
                            <span class="ms-btn">contact</span>
                            <span class="nav-item__label">Get in touch with me</span>
                        </a>
                    </li>
                </ul>
            </div>
        </nav>
    </header>
    <!-- Container -->